...
export default function SupportCard(props: { index: number, delay: number, visible: boolean, support: SupportType }) {
    return <div style={{ transitionDelay: `${props.visible ? props.delay : 0}ms` }} className={`basis-80 flex items-center justify-center flex-none aspect-square ${props.support.bgColor} bg-opacity-90 hover:bg-opacity-100 md:odd:mb-20 md:even:mt-20 relative group transition-all duration-500 ${props.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
        ...
        <div className="z-10 flex flex-col gap-4 text-white py-10 px-lg justify-start h-full">
            <h3 className="text-3xl first-letter:capitalize">{props.support.title} <span className="md:text-5xl">{props.support.titleHighlight}</span></h3>
            <p className="text-md first-letter:capitalize text-lg font-titillium">{props.support.description}</p>
